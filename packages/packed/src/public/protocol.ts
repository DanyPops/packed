export interface PackageEvidence { shape: "keyword-only" | "manifest" | "conventional"; verified: boolean; evidence: string[]; }
export interface PublicationEvidence { integrity?: string; provenanceUrl?: string; trustedPublisher: "verified" | "not-verified" | "unknown"; }
export interface PackageSummary { name: string; version: string; description?: string; date?: string; packageEvidence?: PackageEvidence; publication?: PublicationEvidence; }
export interface PackageInfo extends PackageSummary { homepage?: string; repository?: string; license?: string; keywords?: string[]; pi?: Record<string, unknown>; modified?: string; unpackedSize?: number; readme?: string; readmeAvailable?: boolean; bugs?: string; peerDependencies?: Record<string, string>; }
export interface InstalledPackage { name: string; pinned?: string; installed?: string; }
export interface UpdateEntry { name: string; installed: string; latest: string; detectedAt?: string; }
export interface UpdateOutcome { output: string; reloadRequired: boolean; alreadyUpToDate: boolean; pinned: boolean; previousVersion?: string; currentVersion?: string; }
export interface Diagnostic { code: string; severity: "error" | "warning" | "info"; path: string; message: string; fix?: string; }
export interface SetupOperation { kind: "install-package" | "update-package" | "remove-package" | "write-profile" | "remove-profile"; packageName?: string; name?: string; scope: "global" | "project"; source?: string; resolved?: string; fields?: string[]; }
