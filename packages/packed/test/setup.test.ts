...
class InstallerFixture implements Installer {
	...
	removed: string[] = [];
	fail = false;
	async install(source: string, options?: { local?: boolean }) { this.installed.push(source); if (options?.local) this.projectScoped.push(source); if (this.fail) throw new Error("install failed"); return `Installed ${source}`; }
	async update(source: string): Promise<UpdateOutcome> { this.installed.push(source); return { output: `Updated ${source}`, reloadRequired: true, alreadyUpToDate: false, pinned: true }; }
	async remove(source: string) { this.removed.push(source); return `Removed ${source}`; }
}

class GitFixture implements GitResolutionPort {
	constructor(private commit = "a".repeat(40)) {}
	async resolve(source: string) { return { commit: this.commit, source: `${source.replace(/@[^@/]+$/, "")}@${this.commit}` }; }
}
...
function piHome(withPackage = true): string {
	...
	if (withPackage) {
		...
		writeFileSync(join(root, "npm/node_modules/pi-demo/package.json"), JSON.stringify({ name: "pi-demo", version: "1.2.3", scripts: { postinstall: "touch setup-export-ran-code" } }));
		writeFileSync(join(root, "npm/package-lock.json"), JSON.stringify({ packages: { "node_modules/pi-demo": { integrity: "sha512-demo" } } }));
	}
	return root;
}
...
const manifest: SetupManifest = {
	...
	profiles: { work: { scope: "global", provider: "openai", model: "gpt", thinkingLevel: "high", tools: ["read"], instructions: "Keep work usage scoped.", theme: "dark", allowedModels: ["openai/*"] } },
	defaultProfile: "work",
};
...
describe("Pi setup manifest walking skeleton", () => {
	...
	it("exports one locked npm package and scoped profile canonically", async () => {
		...
		expect(report.manifest.packages).toEqual([{ kind: "npm", scope: "global", source: "npm:pi-demo@1.2.3", resolved: "1.2.3", integrity: "sha512-demo" }]);
	...
	it("plans without mutation and applies locked packages before atomic profile merge", async () => {
		...
		expect(JSON.parse(readFileSync(join(home, "profiles.json"), "utf8"))).toEqual({ personal: { provider: "openai", model: "mini" }, work: expect.objectContaining({ provider: "openai", model: "gpt" }) });
	...
	it("produces an empty plan when locked package and profile state already match", async () => {
		const home = piHome();
		writeFileSync(join(home, "profiles.json"), JSON.stringify({ work: { provider: "openai", model: "gpt", thinkingLevel: "high", tools: ["read"], instructions: "Keep work usage scoped.", theme: "dark", allowedModels: ["openai/*"] } }));
		const root = project();
		writeFileSync(join(root, "pi-setup.json"), JSON.stringify(manifest));
	...
	it("resolves git and HTTPS sources immutably and requires explicit machine-local export", async () => {
		const home = piHome(false);
		writeFileSync(join(home, "settings.json"), JSON.stringify({ packages: ["git:github.com/example/tool@main", "https://github.com/example/other", "./local-tool"] }));
		const root = project();
		...
		expect(report.manifest.packages.find((pkg) => pkg.kind === "local")).toMatchObject({ machineLocal: true, resolved: join(home, "local-tool") });
	});
	...
	it("updates immutable resolutions without applying packages", async () => {
		...
		const changing: SetupManifest = { ...manifest, packages: [{ kind: "git", scope: "global", requested: "git:github.com/example/tool@main", source: `git:github.com/example/tool@${old}`, resolved: old }] };
		...
		expect(result.manifest.packages[0]).toMatchObject({ requested: "git:github.com/example/tool@main", source: `git:github.com/example/tool@${next}`, resolved: next });
		expect(installer.installed).toEqual([]);
	});
	...
	it("plans deterministic prune last and applies removals after desired profile state", async () => {
		...
		expect(JSON.parse(readFileSync(join(home, "profiles.json"), "utf8"))).toEqual({ work: { provider: "openai", model: "gpt", thinkingLevel: "high", tools: ["read"], instructions: "Keep work usage scoped.", theme: "dark", allowedModels: ["openai/*"] } });
	});

	it("repairs an installed npm package whose lock integrity differs", async () => {
		const home = piHome();
		writeFileSync(join(home, "npm/package-lock.json"), JSON.stringify({ packages: { "node_modules/pi-demo": { integrity: "sha512-other" } } }));
		const root = project();
		writeFileSync(join(root, "pi-setup.json"), JSON.stringify(manifest));
	...
	it("rejects mutable git manifests and credential-bearing URLs", () => {
		...
		expect(() => decodeSetupManifest(JSON.stringify({ ...manifest, packages: [{ kind: "git", scope: "global", requested: "git:github.com/example/tool@main", source: "git:github.com/example/tool@main", resolved: commit }] }))).toThrow("SETUP_GIT_NOT_IMMUTABLE");
		expect(() => decodeSetupManifest(JSON.stringify({ ...manifest, packages: [{ kind: "https", scope: "global", requested: "https://github.com/example/tool", source: `https://user:token@github.com/example/tool@${commit}`, resolved: commit }] }))).toThrow("SETUP_CREDENTIAL_SOURCE");
	});

	it("keeps project profiles project-scoped", async () => {
		const home = piHome(false);
		const root = project();
		const value: SetupManifest = { ...manifest, packages: [], profiles: { project: { scope: "project", theme: "dark" } }, defaultProfile: "project" };
