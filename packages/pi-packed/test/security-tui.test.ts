...
function fakeHost(confirm: boolean, notices: string[]): { host: TabHost; renders: number } {
	...
	const host: TabHost = {
		...
		inlineCtx: { ui: { confirm: async (_t: string, message: string) => { notices.push(`confirm:${message}`); return confirm; } } } as unknown as TabHost["ctx"],
...
describe("SettingsTab (/packed's Settings tab -- a real Component, not Pi's native select/confirm)", () => {
	...
	it("never mutates when the confirmation is declined", async () => {
		...
		const natives = {
			...
			async setMutationApproval() { calls += 1; return { mutationApproval: "never" as const }; },
		} as unknown as Natives;
		const tab = await createSettingsTab(natives, host, THEME);
	...
	it("selecting the already-current option is a no-op -- no confirm, no mutation", async () => {
		...
		const natives = {
			async security() { return { mutationApproval: "always" as const }; },
			async setMutationApproval() { throw new Error("must not be called"); },
		} as unknown as Natives;
		const host: TabHost = {
			ctx: { ui: { notify() {} } } as unknown as TabHost["ctx"],
			inlineCtx: { ui: { confirm: async () => { confirmCalled = true; return true; } } } as unknown as TabHost["ctx"],
			requestRender() {},
			onSessionReplaced() {},
		};
		const tab = await createSettingsTab(natives, host, THEME);
